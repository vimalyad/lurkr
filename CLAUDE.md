...
## HOSTED DEMO (LIVE) — supersedes "APK step (PENDING)" above
- Backend hosted on **Render** (free): **https://lurkr-jgdu.onrender.com** (`/health` → `{ok,db:true}`). Env vars set in Render dashboard: `OPENROUTER_API_KEY`, `TAVILY_API_KEY`, `DATABASE_URL` (Neon). Free tier sleeps after ~15 min idle (~50s cold start). Auto-deploys on push to `main` (render.yaml blueprint).
- **APK is DONE** — points at the hosted backend, so it works on any network with the laptop off. Build:
  `VITE_API_URL=https://lurkr-jgdu.onrender.com npm run build && npx cap sync android && cd android && JAVA_HOME="/c/Program Files/Android/Android Studio/jbr" ./gradlew assembleDebug`
- Capacitor 7, appId `com.lurkr.app`, android-35 installed, `usesCleartextTraffic=true` (not needed now that backend is HTTPS, kept harmless). Launcher icon via `npx @capacitor/assets generate` from `assets/icon-only.png`. Header logo `public/lurkr-icon.png`.
- Deploy APK to phone: MTP scripted copy does NOT work on this device (blocked/non-interactive) — use Wi-Fi download (`http://<LAN_IP>:5173/lurkr.apk`), manual Explorer drag, or send `Downloads/lurkr.apk` via any channel. Reinstall = uninstall first (WebView/icon cache).
